...
class Ultimate_ModuleCreator_Model_Attribute_Type_Website extends Ultimate_ModuleCreator_Model_Attribute_Type_Abstract
{
    ...
    public function getRssText()
    {
        ...
        $text      .= '()) '.$eol.$this->getPadding(4).'? Mage::getModel(\'core/website\')->load($item->get';
        $text      .= $this->getAttribute()->getMagicMethodCode();
    ...
    public function getFrontendHtml()
    {
        ...
        $html .= '()) ? Mage::getModel(\'core/website\')->load($_';
        $html .= $entityName.'->get'.$this->getAttribute()->getMagicMethodCode();
    ...
    public function getSetupSource()
    {
        $module     = $this->getModule()->getLowerModuleName();
        $namespace  = $this->getNamespace(true);
        return $namespace.'_'.$module.'/attribute_source_website';
    }
    ...
    public function getFormOptions()
    {
        ...
        $options .="'values'=> Mage::getResourceModel('core/website_collection')->toOptionArray(),";
        $options .= $this->getEol();
        return $options;
    }
    ...
    {
        return 'Mage::getResourceModel(\'core/website_collection\')->toOptionArray()'.$this->getEol();
    }
}
